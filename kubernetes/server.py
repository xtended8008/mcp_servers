...
def list_pods(namespace: str = "default") -> str:
    ...
    try:
        ...
        output = []
        output.append(
            f"{'NAME':<30} {'READY':<10} {'STATUS':<15} {'RESTARTS':<10} {'AGE':<10}"
        )
    ...
    try:
        ...
        if events.items:
            ...
            for event in events.items:
                ...
                output.append(
                    f"{event.type:<10} {event.reason:<20} {age:<10} {event.message}"
                )
        else:
            output.append("<none>")
...
def list_nodes() -> str:
    ...
    try:
        ...
        nodes = v1.list_node()
        output = [
            f"{'NAME':<30} {'STATUS':<15} {'ROLES':<20} {'AGE':<10} {'VERSION':<10}"
        ]
        for node in nodes.items:
            ...
            output.append(
                f"{node.metadata.name:<30} {status:<15} {roles_str:<20} {age:<10} {version:<10}"
            )
        return "\n".join(output)
    except Exception as e:
        return f"Error: {e}"
...
def list_events(namespace: str = "default") -> str:
    ...
    try:
        ...
        for event in sorted_events[:20]:  # Limit to top 20
            ...
            output.append(
                f"{event.type:<10} {event.reason:<20} {obj:<30} {age:<10} {event.message}"
            )
        return "\n".join(output)
    except Exception as e:
        return f"Error: {e}"
...
def list_deployments(namespace: str = "default") -> str:
    ...
    try:
        ...
        output = [
            f"{'NAME':<30} {'READY':<10} {'UP-TO-DATE':<12} {'AVAILABLE':<10} {'AGE':<10}"
        ]
        for d in deps.items:
            ...
            output.append(
                f"{d.metadata.name:<30} {ready:<10} {uptodate:<12} {available:<10} {age:<10}"
            )
        return "\n".join(output)
    except Exception as e:
        return f"Error: {e}"
...
def list_services(namespace: str = "default") -> str:
    ...
    try:
        ...
        output = [
            f"{'NAME':<30} {'TYPE':<15} {'CLUSTER-IP':<15} {'EXTERNAL-IP':<15} {'PORTS'}"
        ]
        for svc in svcs.items:
            ports = (
                ",".join([f"{p.port}/{p.protocol}" for p in svc.spec.ports])
                if svc.spec.ports
                else ""
            )
            ext_ip = "<none>"
            ...
            output.append(
                f"{svc.metadata.name:<30} {svc.spec.type:<15} {svc.spec.cluster_ip:<15} {ext_ip:<15} {ports}"
            )
        return "\n".join(output)
    except Exception as e:
        return f"Error: {e}"
...
def list_helm_releases(namespace: str = "default") -> str:
    ...
    try:
        ...
        output = [
            f"{'NAME':<30} {'NAMESPACE':<15} {'REVISION':<10} {'STATUS':<15} {'CHART':<30} {'APP VERSION'}"
        ]
        for r in releases:
            output.append(
                f"{r['name']:<30} {r['namespace']:<15} {r['revision']:<10} {r['status']:<15} {r['chart']:<30} {r['app_version']}"
            )
